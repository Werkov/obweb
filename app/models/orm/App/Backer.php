 ...
 */
class Backer extends \Navigation\Record implements \Nette\Security\IResource {
    ...
    public static function menuParentInfo($params = array()) {
        $row = self::find(array('user_id' => $params['id']));

        $res[\Navigation\Navigation::PINFO_PARAMS] = array();
        $res[\Navigation\Navigation::PINFO_THIS][\Navigation\Navigation::PINFO_TEXT] = $row->user_id;
    ...
    public static function getPossibleUsers() {
        return \dibi::select(
                        ...
                        ->where("u.active = 1")
                        ->where("(u.registration IS NULL OR m.active = 0)")
