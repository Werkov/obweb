...
abstract class Record extends Storage implements IRecord{
   ...
   /** @var string */
   protected static $mapperClass = 'Ormion\CachingMapper';
   /** @var array */
   ...
   /**
    * Save record
    * @param string oldHash  old hash to compare when updating
    * @param bool to handle cycles in dependency graph
    * @return Record
    */
   public function save($oldHash = null, $asReference = false) {      
	 ...
	 case self::STATE_EXISTING:
	    if ($oldHash !== null) { //check for concurrency	       
	       $actual = $this->getMapper()->find($this->getPrimary(), true);
	       if (!$actual || $actual->getHash() !== $oldHash) {
		  throw new \ModelException("Concurrency issue. Record has changed.", \ModelException::CODE_CONCURRENCY_ISSUE);
	       }
	    }
	    $this->getMapper()->update($this, $asReference);
	    break;

	 case self::STATE_NEW:
	    $this->getMapper()->insert($this, $asReference);
	    break;
      }
      return $this;
   }
   ...
   public function getHash() {
      //TODO: Zahrnout asociace
      $hash = "";
      foreach (parent::getValues() as $value) {
	 if ($value instanceof DateTime)
	    $hash = md5($value->format('Y-m-d H:i:s') . $hash);
	 else
	    $hash = md5($value . $hash);
      }
      return $hash;
   ...
   public function delete() {
      if($this->getState() == self::STATE_DELETED || $this->getState() == self::STATE_NEW){
