...
class Sortable extends \Nette\Object implements IBehavior {
    ...
    public function fixOrderBeforeUpdate(IRecord $record) {
        if ($record->isValueModified($this->orderColumn) || (isset($this->groupColumn) && $record->isValueModified($this->groupColumn))) {
            $original = $record->getMapper()->find($record->getPrimary(), true);
            ...
            //correct range
            if ($record->{$this->orderColumn} < $min)
                $record->{$this->orderColumn} = $min;
            if ($record->{$this->orderColumn} > $max)
                $record->{$this->orderColumn} = $max;

            //first delete-like unset order of the receord
            $fluent = $db->update($record->getMapper()->getTable(), array(
                        $this->orderColumn . "%sql" => array("%n - 1", $this->orderColumn)
                    ))
